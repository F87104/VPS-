...
- 外部公開を有効化した場合、Slackへ10件の候補一覧とダッシュボードURLを通知する
...
6:30、11:45、18:15にXで伸びている投稿を検索し、返信数・リポスト・いいね・表示数・スコアをSlackへ通知します。外部公開を有効化した場合、返信ダッシュボードでは上位10件の通常返信案・大喜利返信案をコピーでき、元ポストも開けます。自動返信は行いません。
